<?php
	...
	/*Modification pseudo*/
	if(!empty(trim($_POST['newpseudo'])) AND $_POST['newpseudo'] != $userinfos['pseudo']) {
	...
	/*Modification email*/
	if(!empty(trim($_POST['newmail'])) AND $_POST['newmail'] != $userinfos['mail']) {
	...
	if(!empty(trim($_POST['newmdp'])) AND sha1($_POST['newmdp']) != $userinfos['motdepasse']) {
